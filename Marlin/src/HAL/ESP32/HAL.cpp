...
#if ENABLED(EXTERNALLIB_WIFISUPPORT)
#include <espExternalLib.h>
...
{
#if ENABLED(EXTERNALLIB_WIFISUPPORT)
    return espExternalLib.parse(command_ptr);
#else
    UNUSED(command_ptr);
    return false;
#endif
...
#if ENABLED(SDSUPPORT) && ENABLED(CONTROL_SD_ACCESS)
bool isSdUsed()
{
#if ENABLED(EXTERNALLIB_WIFISUPPORT)
    return espExternalLib.isSdUsed();
#endif
    return false;
}
#endif
...
void MarlinHAL::idletask()
{
    ...
    OTA_handle();
#endif
    TERN_(EXTERNALLIB_WIFISUPPORT, espExternalLib.idletask());
}
